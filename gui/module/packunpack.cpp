...
int CUnpackPack::SetErrorStatus (int iError)
{
	iErrorStatus = iError;
	return iErrorStatus;
}

int CUnpackPack::GetErrorStatus (void)
{
	return iErrorStatus;
}

void CUnpackPack::ClearErrorStatus (void)
{
	iErrorStatus = 0;
}
...
{
	ClearErrorStatus();
	if (iLenFile <= 0) {SetErrorStatus(-0x01); return 0;} else {}
...
{
	ClearErrorStatus();
	if (iNumByte <= 0) {SetErrorStatus(-0x01); return 0;} else {}
...
int CUnpackPack::DefineTypeFile (void)
{
	ClearErrorStatus();
	iFlagDefSave = 0;
	iFlagDefSaveLang = 0;
	...
	if (memcmp( &aBufferOne[0x0], &iReserved[0x0], 4) != 0) {SetErrorStatus(E_LIBPACK_CHECKCONST); return E_LIBPACK_CHECKCONST;} else {}
	...
	if (iFlagDefSave == 0) {SetErrorStatus(E_LIBPACK_TYPEFILE); return E_LIBPACK_TYPEFILE;} else {}
	return 0;
}
...
int CUnpackPack::CheckFileSignature(void)
{
	...
	) {} else {SetErrorStatus(E_LIBPACK_CHECKCONST); return E_LIBPACK_CHECKCONST;}
	...
	if (CheckCrc32( (Info->data.iLenFile-(0x4+0x4+0x8)), aBufferOne) != Info->data.iCrc32 ) {SetErrorStatus(E_LIBPACK_CHECKCRC32); return E_LIBPACK_CHECKCRC32;} else {}
	
	return 0;
}

int CUnpackPack::Compression(void)
{
	ClearErrorStatus();
	//if (aBufferTemp.capacity() > 1) {
	aBufferTemp.clear();
	//}
	...
	if (deflateInit2(&stream_dec, Z_OVERLORD_SPEED, Z_DEFLATED, iWindowBits, 8, Z_DEFAULT_STRATEGY) != Z_OK) {deflateEnd(&stream_dec); SetErrorStatus(E_LIBPACK_COMPRESS); return E_LIBPACK_COMPRESS;} else {}
	deflateReset(&stream_dec);
	
	if (deflate(&stream_dec, Z_FINISH) != Z_STREAM_END) 
	{deflateEnd(&stream_dec); SetErrorStatus(E_LIBPACK_COMPRESS); return E_LIBPACK_COMPRESS;} else {}
...
int CUnpackPack::Decompression(void)
{
	ClearErrorStatus();
	...
	if ( inflateInit2(&stream_dec, iWindowBits) != Z_OK ) {SetErrorStatus(E_LIBPACK_DECOMPRESS); return E_LIBPACK_DECOMPRESS;} else {}
	inflateReset(&stream_dec);

	if (inflate(&stream_dec, Z_NO_FLUSH) != Z_STREAM_END) {SetErrorStatus(E_LIBPACK_DECOMPRESS); return E_LIBPACK_DECOMPRESS;} else {}
	inflateEnd(&stream_dec);
	
	if ( CheckSum(UnionDataInfoOne.data.iUnzip, aBufferTemp) != UnionDataInfoOne.data.iChecksum) {SetErrorStatus(E_LIBPACK_CHECKSUM); return E_LIBPACK_CHECKSUM;} else {}
