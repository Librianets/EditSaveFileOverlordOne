...
BOOL CheckFileSignature(void)
{
	uniDInfo.info.iConst1 = 0xDEADBEEF;
	uniDInfo.info.iConst2 = 0xFEEDDEAF;
	uniDInfo.info.iReserved = 0x00000002;
	uniDInfo.info.iSignature = 0x5E78;
	
	if 
	(
	memcmp(&aGlobalBuffer[uniDInfo.info.iLenFile-12], &uniDInfo.info.iConst2, 4) 	== 0 &&
	memcmp(&aGlobalBuffer[uniDInfo.info.iLenFile-16], &uniDInfo.info.iConst1, 4) 	== 0 &&
	memcmp(&aGlobalBuffer[uniDInfo.info.iLenFile-4], &uniDInfo.info.iReserved, 4) 	== 0 &&
	memcmp(&aGlobalBuffer[4], &uniDInfo.info.iSignature, 2) 						== 0
	) {log(L"File check ok");} else {return FALSE;}
	
	memcpy (&uniDInfo.info.iCrc32, &aGlobalBuffer[uniDInfo.info.iLenFile-8], 4);
	memcpy (&uniDInfo.info.iChecksum, &aGlobalBuffer[uniDInfo.info.iLenFile-18], 4);
	memcpy (&uniDInfo.info.iUnzip, &aGlobalBuffer[0], 4);
	
	log(L"info.iUnzip=%i", 		uniDInfo.info.iUnzip);
	log(L"info.iCrc32=%x", 		uniDInfo.info.iCrc32);
	log(L"info.iChecksum=%x",	uniDInfo.info.iChecksum);
	log(L"info.iLenFile=%i", 	uniDInfo.info.iLenFile);
	
	if (CheckCrc32((uniDInfo.info.iLenFile-(0x4+0x4+0x8)), &aGlobalBuffer[0x0], table) != uniDInfo.info.iCrc32) {return FALSE;}
...
void decompression(size_t iUnzip, size_t iLenFile, void* pAddressIn, void* pAddressOut)
{
	z_stream stream_dec;
	...
	if ( inflateInit2(&stream_dec, windowBits) != Z_OK ) {}
	inflateReset(&stream_dec);
	
	stream_dec.avail_in = (iLenFile-0x6-0x4-0x4-0x4-0x8);
	stream_dec.avail_out = iUnzip;
	//uniDInfo.info.iUnzip, uniDInfo.info.iLenFile, &aGlobalBuffer[0x0], &aBufferDecomp[0x0]
	memset(aBufferDecomp, 0, 0x10000);
	//log(L"pAddressIn = %x", (unsigned char*) pAddressIn);	
	stream_dec.next_in = &aGlobalBuffer[0x6];
	stream_dec.next_out = aBufferDecomp;
	
	int status_dec = 255;
	status_dec = inflate(&stream_dec, Z_NO_FLUSH);
	log(L"status_dec = %i", status_dec);	
	log(L"msg = %s", stream_dec.msg);	
		log(L"Z_STREAM_END  = %i", Z_STREAM_END );	
